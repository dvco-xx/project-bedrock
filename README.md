...
I created a GitHub repo for the project and cloned it via https,
...
Next, I installed the Helm dependency and created a namespace:

- kubectl create namespace retail-store

Then cloned the app from the awscontainers github repo:
...
I then retrieved the external URL [http://a63232a95b7094296a6343a855930427-358081964.us-east-1.elb.amazonaws.com/] and opened it in my browser.
...
Then I created a developer-rbac.yaml to give this user read-only access:

- kubectl apply -f kubernetes/developer-rbac.yaml

This created a ClusterRole named "developer-readonly"

- ClusterRoleBinding to "developer-group"
- Permissions: get, list, watch on pods, services, deployments, etc.
...
But it restricts their access to delete or modify anything.
...
This was the key fix from previous attempts. The aws-auth ConfigMap maps IAM users to Kubernetes identities.

I ran the following commands to patch the aws-auth ConfigMap and add the developer user mapping:

- DEVELOPER_ARN=$(cd terraform && terraform output -raw developer_arn && cd ..)

- kubectl patch configmap aws-auth -n kube-system --type merge -p \
  "{\"data\":{\"mapUsers\":\"- username: project-bedrock-developer\n groups:\n - developer-group\n userarn: $DEVELOPER_ARN\n\"}}"

Then I verified the mapping was added using:

- kubectl get configmap aws-auth -n kube-system -o yaml | grep -A 5 "mapUsers"

I then tested the developer access using:

- export AWS_ACCESS_KEY_ID=$(cd terraform && terraform output -raw developer_access_key_id && cd ..)
export AWS_SECRET_ACCESS_KEY=$(cd terraform && terraform output -raw developer_secret_access_key && cd ..)

Updated kubeconfig with developer credentials

- aws eks update-kubeconfig --region us-east-1 --name project-bedrock-cluster

And tested read access

- kubectl get pods -A

Tested write access by trying to delete a pod (which failed, as it should)

- kubectl delete pod test-pod -n default

Then switched back to admin credentials
...
Then I added my AWS credentials (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY) as repository secrets in the GitHub Secrets settings page.

Step 8:
Implementing GitFlow Branching

---

I set up efficient branching by creating a branch for development (develop):
...
- Deprecated GitHub Actions versions which prevented the actions file from running (I had to update from v3 to v4),
- Did not use the aws-auth ConfigMap which maps IAM users to Kubernetes identities (now fixed),
...
- AWS RDS databases (postgres and sql)
- ALB and Ingress
